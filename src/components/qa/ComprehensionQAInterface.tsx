...
export const ComprehensionQAInterface: React.FC<QAInterfaceProps> = ({ stats, mostPlayedGame, topPerformers, recentGames }) => {
  return (
        ...
        <div className="bg-gradient-to-br from-gray-900/90 to-cyan-900/80 rounded-2xl p-6 backdrop-blur border-2 border-cyan-500/50 shadow-xl">
          ...
          <div className="space-y-3">
            {topPerformers.map((player, i) => (
              <div key={i} className={`flex items-center gap-4 p-4 rounded-xl transition-all hover:scale-102 ${i === 0 ? 'bg-gradient-to-r from-teal-500/30 to-cyan-500/30 border-2 border-teal-400/60 shadow-lg shadow-teal-500/30' :
                ...
                <div className={`w-14 h-14 rounded-xl flex items-center justify-center font-bold text-2xl shadow-lg relative ${i === 0 ? 'bg-gradient-to-br from-teal-400 to-cyan-500 text-black' :
